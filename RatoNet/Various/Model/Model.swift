...
private func randomBuyIconsTitle() -> String {
    return [
        String(localized: "👍 Buy RatoNet icons if you like the app 👍"),
        String(localized: "🍔 Buy RatoNet icons to support the devs 🍔"),
        String(localized: "🙈 Buy RatoNet icons to hide this message 🙈"),
        String(localized: "🙏 Buy RatoNet icons please =) 🙏"),
    ].randomElement()!
}
...
final class Model: NSObject, ObservableObject, @unchecked Sendable {
    ...
    func formatLog(log: Deque<LogEntry>) -> URL {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("RatoNet-log-\(Date())")
    ...
    func setup() {
        ...
        RatoNetShortcuts.updateAppShortcutParameters()
    ...
    private func setupSampleBufferReceiver() {
        ...
        sampleBufferReceiver.start(appGroup: ratonetAppGroup)
    }
    ...
    func updateFaceFilterSettings() {
        ...
        faceEffect.setSettings(settings: FaceEffectSettings(
            ...
            showMouth: settings.showRatoNet
        ))
    }
    ...
    func updateFaceFilterButtonState() {
        ...
        if database.debug.face.showRatoNet {
            isOn = true
        }
    ...
    func startWorkout(type: WatchProtocolWorkoutType) {
        guard WCSession.default.isWatchAppInstalled else {
            makeToast(title: String(localized: "Install RatoNet on your Apple Watch"))
            return
        }
        ...
        makeToast(
            ...
            subTitle: String(localized: "Open RatoNet in your Apple Watch to start it")
        )
    }
    ...
    func stopWorkout(showToast: Bool = true) {
        ...
        if showToast {
            makeToast(title: String(localized: "Ending workout"),
                      subTitle: String(localized: "Open RatoNet in your Apple Watch to end it"))
        }
    }
    ...
    func findScoreboardPlayer(id: UUID) -> String {
        return database.scoreboardPlayers.first(where: { $0.id == id })?.name ?? "🇸🇪 RatoNet"
    }
        ...
        if battery.level <= 0.07, !isBatteryCharging(), !isMac() {
            ...
            if (battery.levelLowCounter % 3) == 0 {
                ...
                if database.chat.botEnabled, database.chat.botSendLowBatteryWarning {
                    sendChatMessage(message: "RatoNet bot: \(lowBatteryMessage)")
                }
            }
        } else {
            battery.levelLowCounter = -1
