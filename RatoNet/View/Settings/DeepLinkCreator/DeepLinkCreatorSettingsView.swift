...
private let defaultDeepLink = "ratonet://?{}"
...
struct DeepLinkCreatorSettingsView: View {
    ...
    private func createDeepLinkStream(stream: DeepLinkCreatorStream) -> RatoNetSettingsUrlStream {
        let newStream = RatoNetSettingsUrlStream(name: stream.name, url: stream.url)
        if stream.selected {
    ...
    private func updateDeepLinkStreams(settings: RatoNetSettingsUrl) {
    ...
    private func updateDeepLinkQuickButtons(settings: RatoNetSettingsUrl) {
        ...
        for button in deepLinkCreator.quickButtons.buttons where button.enabled {
            ...
            let newButton = RatoNetSettingsButton(type: button.type)
            newButton.enabled = true
            settings.quickButtons!.buttons!.append(newButton)
        }
    }

    private func updateDeepLinkWebBrowser(settings: RatoNetSettingsUrl) {
    ...
    private func updateDeepLink() {
        let settings = RatoNetSettingsUrl()
        ...
        do {
                ...
                .addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryAllowed)
            {
                ...
                deepLink = "ratonet://?\(encodedJsonBlob)"
            }
        } catch {
