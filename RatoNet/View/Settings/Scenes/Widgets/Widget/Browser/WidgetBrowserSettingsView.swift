...
struct WidgetBrowserSettingsView: View {
    ...
    var body: some View {
        ...
        Section {
            Toggle("RatoNet access", isOn: $browser.ratonetAccess)
                .onChange(of: browser.ratonetAccess) { _ in
                    model.resetSelectedScene(changeScene: false)
                }
        } footer: {
            Text("Give the webpage access to various data in RatoNet, for example chat messages and your location.")
        }
